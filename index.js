...
function findElementGreaterThan(arr, num) {
  if (Array.isArray(arr)) {
    let value = arr.find((elem) => elem > num);
    if (value) {
      return value;
    } else {
...
function findIndexByProperty(arr, name) {
  if (Array.isArray(arr)) {
    let foundIndex = arr.findIndex((elem) => {
...
function findLastEvenNumber(arr) {
  if (Array.isArray(arr)) {
    let lastEvenNumber = arr.findLast((elem) => elem % 2 === 0);
    if (lastEvenNumber) {
...
function getSubArrayAndConvertToString(arr, startIdx) {
  if (Array.isArray(arr)) {
    let subArrayString = arr.splice(startIdx);
...
function getArrayKeys(arr) {
  if (Array.isArray(arr)) {
    let keysObj = arr.keys();
    let arrayKeys = Array.from(keysObj);
    return arrayKeys;
  } else {
    return "Вхідний параметр має бути масивом";
  }
...
function getPositiveArrayValues(arr) {
  if (Array.isArray(arr)) {
    ...
    let newArr = Array.from(iterator);
    let positiveArrayValues = newArr.filter((elem) => elem > 0);
...
function removeAndDouble(arr, startIdx) {
  if (Array.isArray(arr)) {
    arr.splice(startIdx);
    let doubledArray = arr.map((elem) => elem * 2);
    return doubledArray;
  } else {
    return "Вхідний параметр має бути масивом";
  }
...
function getLastElementsSumAndPrint(arr, n) {
  if (Array.isArray(arr)) {
    let arrPart = arr.splice(n + 1);
    let sum = 0;
    let sumArr = arrPart.reduceRight(
      (accumulator, curValue) => accumulator + curValue,
      sum
    );
    arrPart.forEach((elem) => console.log(elem));
    return sumArr;
  } else {
    return "Вхідний параметр має бути масивом";
  }
...
function findLongestWord(arr) {
  if (Array.isArray(arr)) {
    let longestWord = "";
    arr.reduce((lastWord, currentWord) => {
...
function findDuplicateElements(arr) {
  if (Array.isArray(arr)) {
    let duplicateElements = arr.filter((elem, index) => {
...
function findLastNegativeNumberIndex(arr) {
  if (Array.isArray(arr)) {
    let lastIndex = arr.findLastIndex((elem) => elem < 0);
    return lastIndex;
  } else {
    return -1;
  }
